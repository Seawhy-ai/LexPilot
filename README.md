...
| 🎯 **学习题库** | 493 道法学题目，宪法/民法/刑法按马工程教材章节分组，每题标注难度，练习 / 考试双模式，错题本 |
...
- **v2.6.1** (2026.08.11) — 临时移除登录界面（代码保留，可直接进主页）；新增 iOS 安装包支持：仓库内置 Capacitor iOS 工程与 GitHub Actions 云构建，可在 Actions 一键产出无签名 .ipa，用 Apple ID + Sideloadly 签名安装；清理仓库中误提交的 config.json（含 API Key）与旧备份文件
- **v2.6.0** (2026.08.11) — 题库升级至 493 题并按马工程教材章节重组、标注难度；题库主界面改为练习/考试/错题本三入口，考试模式可具体到章节选题；单选点击即判题、上一页/下一页固定屏幕底部；修复练习返回直达学习中心、打卡状态与主页头像不显示等 bug
